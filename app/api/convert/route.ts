...
`
  ...
  private cleanAndFormatText(text: string): { cleanedText: string; style: string } {
    if (!text) {
  ...
  private parseSRTBlock(block: string): SubtitleEntry | null {
    try {
      const lines = block
        .trim()
        .split("\n")
  ...
  private parseSRTFile(content: string): SubtitleEntry[] {
    try {
      if (!content.trim()) {
  ...
  private validateSubtitleTiming(subtitles: SubtitleEntry[]): SubtitleEntry[] {
  ...
  private writeASSFile(subtitles: SubtitleEntry[]): string {
    try {
  ...
  public convertToASS(srtContent: string): {
    success: boolean
    content?: string
    error?: string
